...
auto largest_prime_factor(int64_t n) {
  auto largest = 1;
  auto all = ranges::views::ints(0, ranges::unreachable) |
             ranges::views::take(boost::math::max_prime) |
             ranges::views::transform(prime) |
             ranges::views::filter([&largest, &n](auto prime) {
               if (n % prime == 0) {
                 largest = prime;
                 n /= prime;
                 return true;
             ...
             ranges::views::take_while([n](auto prime) { return prime < n; }) |
             ranges::to_vector;
