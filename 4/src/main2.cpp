...
template <int order> bool is_product_of_2_n_digits_numbers(int n) {
  auto first_valid_decomposition =
      ranges::views::ints(min_n_digit_number<order>(),
                         max_n_digit_number<order>() + 1) |
      ranges::views::reverse | ranges::views::filter([&n](auto factor) {
        if (n % factor == 0) {
...
template <int order> auto largest_palindrome_product() {
  ...
  auto any = ranges::views::ints(0, max_product + 1) | ranges::views::reverse |
             ranges::views::filter(is_palindrome) |
             ranges::views::filter(is_product_of_2_n_digits_numbers<order>) |
             ranges::views::take(1) | ranges::to_vector;
  return any.front();
}
